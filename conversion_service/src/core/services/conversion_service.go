...
type ConversionService struct {
	...
	rateCache    ports.ConversionRateProvider
	payloadStore ports.PayloadStore
}

func NewConversionService(repo ports.TransactionRepository, rateCache ports.ConversionRateProvider,
	payloadStore ports.PayloadStore) *ConversionService {
...
func (s *ConversionService) GetConvertedTransaction(ctx context.Context, id uuid.UUID,
	targetCurrency string) (ports.TransactionResponseDTO, error) {
	...
	if targetCurrency != "" && targetCurrency != "USD" {
		log.Printf("[ConversionService] Fetching exchange rate for %s on %s",
			targetCurrency, tx.TransactionDate.Format("2006-01-02"))

		rate, err := s.rateCache.GetRate(ctx, targetCurrency, tx.TransactionDate)
		if err != nil {
			log.Printf("[ConversionService] [ERROR] Failed to fetch rate for %s: %v",
				targetCurrency, err)
		...
		resp.Message = fmt.Sprintf("Successfully converted %s USD to %s %s",
			tx.Amount.String(), convertedAmount.String(), targetCurrency)

		log.Printf("[ConversionService] Success: %s USD -> %s %s (Rate: %s)",
			tx.Amount, convertedAmount, targetCurrency, rate.ExchangeRate)
	}
...
func (s *ConversionService) saveResult(ctx context.Context, statusKey, resultKey string, resp ports.TransactionResponseDTO) {
